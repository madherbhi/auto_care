...
class _RegisterPageState extends State<RegisterPage> {
  ...
  final List<BankCode> _bankCodes = [];
  bool _isLoadingBanks = false;
  String? _bankFetchError;

  List<String> get _banks {
    return _bankCodes.map((bank) => bank.bankName).toSet().toList()..sort();
  }

  List<String> get _bankIdOptions => _selectedBank == null
      ? []
      : _bankCodes
  ...
  Future<void> _loadBanks() async {
    ...
    try {
      final banks = await _authController.fetchBankCodes();
      if (!mounted) return;
      setState(() => _bankCodes.addAll(banks));
    } catch (_) {
      if (!mounted) return;
      final message = _authController.errorMessage.value;
      setState(() {
        _bankFetchError = (message == null || message.isEmpty)
  ...
  Widget build(BuildContext context) {
    ...
    final bankIdHint = _isLoadingBanks
        ? 'Loading bank IDs...'
        : bankIdItems.isEmpty
        ? StringHelper.selectBankFirst
        : StringHelper.selectBankId;
    final bankHint = _isLoadingBanks
        ? 'Loading banks...'
        : StringHelper.selectBank;
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              AuthLoginDropdown(
                                ...
                                validator: (v) {
                                  if (_isLoadingBanks) {
                                    return 'Please wait while banks are loading';
                                  }
                                  if (_banks.isEmpty) {
                                    return _bankFetchError ??
                                        'No banks available. Please retry.';
                                  }
                                  return VehicleValidator.required(
                                    v,
                                    message: StringHelper.bankRequired,
                                  );
                                },
                              ),
                              if (_bankFetchError != null) ...[
                                const Gap(8),
                                Align(
                                  alignment: Alignment.centerLeft,
                                  child: TextButton.icon(
                                    onPressed: _isLoadingBanks
                                        ? null
                                        : _loadBanks,
                              ...
                              AuthLoginDropdown(
                                ...
                                onChanged:
                                    _isLoadingBanks || bankIdItems.isEmpty
                                    ? (_) {}
                                    : (id) =>

...
class _VehicleDetailPageState extends State<VehicleDetailPage> {
  ...
  void initState() {
    super.initState();
    _controller = Get.put(AddVehicleController(), tag: 'vehicle_edit');
  }

  void _onUpdate() => _formBodyKey.currentState?.submit();

  Future<void> _onSubmit(VehicleRecord record) async {
    final updated = await _controller.updateCase(
    ...
    Navigator.of(context).pop<CaseModel>(updated);
  }
                ...
                Obx(
                  () => VehicleFormSaveBar(
                    label: _controller.isSubmitting.value
                        ...
                        : StringHelper.update,
                    onPressed:
                        _controller.isSubmitting.value ? null : _onUpdate,
                  ),
                ),
              ],
            ),
